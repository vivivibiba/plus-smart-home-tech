...
public class HubEventProcessor implements Runnable {
    ...
    public HubEventProcessor(@Qualifier("hubEventConsumer") Consumer<String, HubEventAvro> consumer,
                             ...
                             @Value("${analyzer.kafka.hub-events-topic}") String topic,
                             @Value("${analyzer.kafka.hub-consumer.poll-timeout-ms:1000}") long pollTimeoutMs) {
    ...
    public void run() {
        ...
        log.info("Hub event processor subscribed to {}", topic);
        try {
            while (running) {
                for (ConsumerRecord<String, HubEventAvro> record : consumer.poll(pollTimeout)) {
                    if (!process(record)) {
                        break;
                    }
                }
    ...
    private boolean process(ConsumerRecord<String, HubEventAvro> record) {
        ...
        try {
            ...
            log.error("Failed to process hub event at {}-{} offset {}. The record will be retried",
